...
final readonly class MarkedClassesAreOnlyInvokableRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        if (!$hasInvokableMethod) {
            $errors[] = RuleErrorBuilder::message(\sprintf('The class %s has "__invoke" method', $className))
    ...
    private function isEligible(\ReflectionClass $reflection): bool
    {
        ...
        foreach ($this->classes as $class) {
            if (is_a($reflection->getName(), $class, allow_string: true) && !$reflection->isAbstract()) {
                return true;
            }
        }
