...
final class DoctrineServiceCategoryRepositoryTest extends KernelTestCase
{
    ...
    public function testAdd(): void
    {
        ...
        $serviceCategory = $this->repository->findOneByName(new ServiceCategoryName('NAME'));

        self::assertNotNull($serviceCategory);

        $serviceCategory->id();
        $this->addToAssertionCount(1); // it should have thrown an exception otherwise

        self::assertEquals(new ServiceCategoryName('NAME'), $serviceCategory->name);
        self::assertEquals(new ServiceCategoryName('ALIAS'), $serviceCategory->alias);
        self::assertTrue($serviceCategory->activated);
    }
